...
def wrangle_to_csv_mk2(log_dir, data_dir): 
    # Import(s)
    import os, shutil, re
    import numpy as np
    import pandas as pd
    ...
    log_files = np.array([
        os.path.join(log_dir, 
        file) for file in os.listdir(log_dir) if 'errorlog' not in file and '.gitkeep' not in file])
    ...
    for log_file in log_files: 
        ...
        with open(log_file, 'r') as f:
            ...
            for line in f: 
                ...
                elif '# Null hypothesis probability' in line: 
                    dof = float(split_line(line)[7])
...
wrangle_to_csv_mk2(logs_dir,data_dir)
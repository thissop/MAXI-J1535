...
'''
tbfeo_v3()=
    MODEL:  tbfeo(simpl(diskbb)+relxill) 
    DESCR:  This is is the fitting routine for what will hopefully be the best 
            processing of the NICER MAXI J1535-571 data yet. This function incorporates
            second generation methods like using requested arf and rsp files. This function
            does not produce output fit images however. The cflux() command finds the 
            total unabsorbed flux of the model (by placing cflux to the right of tbfeo) and
            logs both the log10flux as well as its confidence interval in the cflux+++++++++.log
            file. 
...
def tbfeo_v3(IDs,pathtemp,outfile):
    ...
    def relxill(fitpathtemp):
        ...
        f.write('3.6 0.01 1.5 1.501 4.5 4.501'+'\n')
        f.write('2 0.01 1.3 1.3 3.4 3.4'+'\n')
...
#get ids
id_list = []
with open('/home/thaddaeus/FMU/Steiner2.0/permanent/>75percentsource.txt','r') as f:
    for line in f:
        if '#' not in line:
            line = line.replace('\n','')
            linelist = line.split(',')
            line = linelist[0]
            if os.path.exists('/home/thaddaeus/FMU/Steiner2.0/20November/Ragnarok/logs/'+line+'.log') == True:
                id_list.append(line)

#tbfeo_v3(IDs=id_list,pathtemp=nicer_file_temp,outfile=fit_commands_file)
from personalastropy.xspectools.XSPECtools import returnFitResults,returnDates,returnHardnessRatios
import matplotlib.pyplot as plt 

hjds = []
returnDates(IDs=id_list,path_temp='/home/thaddaeus/FMU/Steiner/thaddaeus/++++++++++/jspipe/js_ni++++++++++_0mpu7_silver_GTI0.jsgrp',out_list=hjds)
...
returnFitResults(IDs=id_list,path_temp='/home/thaddaeus/FMU/Steiner2.0/20November/Ragnarok/logs/++++++++++.log',fit_stat='pgstat',fit_params=['simpl:Gamma:5','simpl:FracSctr:5','diskbb:Tin:6'],out_lists=[gammas,scat_fracs,disk_temps],rfsl=red_pgs)

#get lists ready for plotting
gammas2 = []
scat_fracs2 =[]
disk_temps2 = []
red_pgs2 = []
...
for item in gammas:
    item = item.split(':')[1]
    gammas2.append(float(item))
for item in scat_fracs:
    item = item.split(':')[1]
    scat_fracs2.append(float(item))
for item in disk_temps:
    item = item.split(':')[1]
    disk_temps2.append(float(item))
for item in red_pgs:
    item = item.split(':')[1]
    red_pgs2.append(float(item))
for item in hjds:
    item = item.split(':')[1]
    hjds2.append(float(item))


fig, axs = plt.subplots(4,1,sharex=True)

axs[0].scatter(hjds2,gammas2,label='Gamma')
axs[0].legend()
axs[1].scatter(hjds2,scat_fracs2,label='Scat. Fracs')
axs[1].legend()
axs[2].scatter(hjds2,disk_temps2,label='Disk Temps')
axs[2].legend()
axs[3].scatter(hjds2,red_pgs2,label='red pgs.')
axs[3].legend()
plt.show()